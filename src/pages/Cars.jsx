...
const Cars = () => {
  ...
  const selectPageHandler = (index) => {
    if(index >= 1 && index <= Math.ceil(CarsData.length/6) && index !== page){
        navigate(`/cars/${index}`);
    }
  }
    ...
    <>
      <div className='bg-gray-500 bg-center h-max p-2 sm:p-12' >
        <div className='sm:flex flex-wrap justify-evenly py-1'>
          {CarsData.slice(page*6 - 6, page*6).map((project, index) => (
        ...
        <div className="  relative" >
            <div className="inline">
                <span>{6*page - 5} - {6*page} of {CarsData.length}</span>
            </div>
            <div className="page float-right mr-16" >
            {
                CarsData.length > 0 && (
                    ...
                    {
                        [...Array(Math.ceil(CarsData.length/6))].map((_, index) => {
                        return (
